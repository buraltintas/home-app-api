...
- local-first search: `SEARCH_LOCAL_FIRST_ENABLED` decides whether the provider is asked on every search or only when our own catalogue cannot answer it. It is off by default, and while it is off the search behaves exactly as it did before the gate existed. The thresholds are `SEARCH_GATE_MIN_RESULTS`, `SEARCH_GATE_MIN_RELEVANCE`, `SEARCH_GATE_RELEVANCE_SAMPLE`, `SEARCH_GATE_MIN_COVERAGE` and `SEARCH_GATE_COVERAGE_RADIUS_METERS`; `SEARCH_SHADOW_RATE` is the fraction of local-only searches measured against the provider afterwards. All have defaults, so none has to be set; they exist to be moved on evidence rather than on a release. Raising a threshold buys safety with provider calls, lowering it does the reverse
