...
func (p *OpenAIParser) parseSearchIntent(ctx context.Context, query string, c Context) (Intent, error) {
	...
	// Structured output constrains the classification shape. GPT-5 mini rejects the
	// temperature sampling parameter, so sending it would force every configured request
	// onto the deterministic fallback before intent enrichment can run.
...
func intentPrompt(query string, c Context) string {
	return fmt.Sprintf(`Parse and classify this physical-store search. The input may be Turkish, English, German, or Russian. Set scope to home_living when the user wants to discover or buy home/living products or stores, including indirect needs such as dowry shopping, refreshing a room, bedding, curtains, furniture, lighting, decoration, kitchenware, bathroom products, carpets, cabinets, tableware, and household accessories. Electrical or lighting supply retailers that sell lamps, fixtures, chandeliers, or home lighting are home_living with category lighting; electrician, wiring, installation, repair, or utility-service requests alone are out_of_scope. A query may consist only of a home/living store or brand name (for example IKEA, Madame Coco, or Yeğenler Elektrik Antalya); classify it as home_living and copy the store or brand name into store_name without translating or shortening it, but keep a trailing or leading city/district only in location_text rather than store_name. Also extract store_name when it appears alongside products or a location. Use at most three categories, and only the ones the query clearly implies; for a bare store or brand name leave categories empty instead of listing everything that store might sell. Set scope to out_of_scope for an explicit unrelated product, service, or business such as a tire shop, restaurant, pharmacy, hairdresser, or electrician service. Set scope to unclear for greetings, chitchat, nonsense, or an intent that cannot be confidently understood. For out_of_scope or unclear, leave store_name, categories, product_terms, style_terms, price_intent, attributes, and semantic_terms empty. Detect query_language as tr, en, de, or ru. Preserve proper nouns and the original query exactly, and map meaning into one canonical schema regardless of language. Categories must use canonical slugs; product_terms, style_terms, and semantic_terms must use concise stable English-like concept keys, never translated display copy. Do not invent coordinates. Requested response locale=%s. Original query=%q`, c.Locale, query)
}
