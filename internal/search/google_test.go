...
func TestGooglePlacesContract(t *testing.T) {
	...
	client := &http.Client{Timeout: time.Second, Transport: placesRoundTripFunc(func(r *http.Request) (*http.Response, error) {
		...
		case "/places:searchText":
			...
			body = `{"places":[{"id":"place-1","displayName":{"text":"Test Mağaza"},"formattedAddress":"Kadıköy, İstanbul","location":{"latitude":40.99,"longitude":29.03},"rating":4.4,"userRatingCount":12,"nationalPhoneNumber":"123","websiteUri":"https://example.test","types":["furniture_store"],"businessStatus":"CLOSED_PERMANENTLY"}]}`
		case "/places/place-1":
			if r.Header.Get("X-Goog-FieldMask") == googleLocationFieldMask {
				...
				body = `{"ID":"place-1","DisplayName":{"Text":"Test Mağaza"},"FormattedAddress":"Kadıköy, İstanbul","Location":{"Latitude":40.99,"Longitude":29.03},"Rating":4.4,"UserRatingCount":12,"Types":["furniture_store"],"NationalPhoneNumber":"123","WebsiteUri":"https://example.test","BusinessStatus":"CLOSED_PERMANENTLY"}`
			}
		default:
	...
	if err != nil || len(places) != 1 || places[0].PlaceID != "place-1" || places[0].RatingCount != 0 || places[0].Rating != 0 || places[0].Phone != "" || places[0].Website != "" || places[0].BusinessStatus != "CLOSED_PERMANENTLY" {
		t.Fatalf("text search places=%+v err=%v", places, err)
	}
	for _, forbidden := range []string{"rating", "userRatingCount", "regularOpeningHours", "utcOffsetMinutes", "nationalPhoneNumber", "websiteUri"} {
	...
	if err != nil || detail.PlaceID != "place-1" || detail.Name != "Test Mağaza" || detail.BusinessStatus != "CLOSED_PERMANENTLY" || !detail.DetailsFetched || detail.Phone != "123" || detail.Website != "https://example.test" {
