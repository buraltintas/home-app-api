...
func (s *Service) AggregateDay(ctx context.Context, day time.Time) error {
	...
	SELECT $1,
	 ...
	 (SELECT count(*) FROM stores WHERE source_kind='brand_locator' AND created_at<$3 AND (deleted_at IS NULL OR deleted_at>=$3)),
