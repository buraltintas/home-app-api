...
func (s *Service) AggregateDay(ctx context.Context, day time.Time) error {
	...
	_, e = tx.Exec(ctx, `WITH ds AS (SELECT parsed_intent FROM searches WHERE created_at>=$2 AND created_at<$3),dims AS (SELECT 'category' dimension,jsonb_array_elements_text(CASE WHEN jsonb_typeof(parsed_intent->'categories')='array' THEN parsed_intent->'categories' ELSE '[]'::jsonb END) value FROM ds UNION ALL SELECT 'product',jsonb_array_elements_text(CASE WHEN jsonb_typeof(parsed_intent->'product_terms')='array' THEN parsed_intent->'product_terms' ELSE '[]'::jsonb END) FROM ds UNION ALL SELECT 'style',jsonb_array_elements_text(CASE WHEN jsonb_typeof(parsed_intent->'style_terms')='array' THEN parsed_intent->'style_terms' ELSE '[]'::jsonb END) FROM ds UNION ALL SELECT 'location',nullif(parsed_intent->>'location_text','') FROM ds UNION ALL SELECT 'price_intent',nullif(parsed_intent->>'price_intent','') FROM ds) INSERT INTO search_intent_daily_metrics(metric_date,dimension,value,search_count) SELECT $1,dimension,value,count(*) FROM dims WHERE value IS NOT NULL GROUP BY dimension,value`, metricDate, start, end)
	if e != nil {
		return s.failRun(ctx, runID, e)
	}
