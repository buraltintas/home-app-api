...
func TestJSONLocatorMapsAPublishedListToRows(t *testing.T) {
	...
	// The adapter hands back what the publisher wrote, whitespace tidied and nothing else.
	// Repairing its capital I's, taking the chain's city code off the front, casing it and
	// adding the town all happen in the importer, in that order, and each of the first two
	// needs the name still shouted.
	if rows[0].Name != "ANK 365 1 AVM" {
