...
- Manual entry has exactly one user-editable text field. The user types a Turkish place name such as `Kadıköy` or `Çankaya` (the table is Turkey only); never show latitude/longitude inputs. Debounce after at least two characters, call `GET /v1/locations/search?q=<text>&limit=5`, and render the candidate's name and the places above it (`Caferağa` / `Kadıköy, İstanbul`); `attributions` is now always empty and nothing needs crediting. See [`location-search.json`](./frontend-fixtures/location-search.json). After selection, call `PUT /v1/me/discovery-location` with only `{"source":"manual","place_id":"..."}`. The backend re-fetches and verifies the geographic place and stores its coordinates privately; never copy candidate coordinates into the manual-selection request, render numeric fields, or ask the user to type them.
...
- Mobile current-location mode has priority: persist OS coordinates and horizontal accuracy with `source=device` while the app is foregrounded. Every accepted device update replaces the stored manual fallback. Update after meaningful movement (recommended: at least 250 m) or when the last persisted fix is at least 15 minutes old; do not request background location. A manual change persists immediately but is used only until the app obtains its next valid device fix.
...
- Manual location is discovery context only. It must never be submitted to `/visit-verifications` or used as evidence for a review. Review verification always requires fresh mobile OS geolocation and horizontal accuracy while the user is physically present.
...
The backend, not frontend, optionally asks OpenAI to parse intent, runs deterministic fallback parsing, queries internal/PostGIS first and Google Places only when the local sufficiency gate requires it, deduplicates/enriches, ranks, and returns structured data. A named store already present locally never invokes Places. The frontend never calls OpenAI or Places and never receives their keys/prompts.
...
For `home_living`, parsed intent always drives the internal/PostGIS query first. Google Text Search runs only when a missing named store or insufficient generic catalogue answer needs it, and requests only identity/classification/list fields without photos. Opening a store is the demand signal for the one-time full detail lookup; the completed result, including an empty optional answer, is persisted and every later detail read is database-only. Indirect requests such as “çeyiz almak istiyorum” and “nevresim takımı lazım” are valid home/living searches, not guidance states. Resolving a manual discovery-location selection uses an Essentials-only Place Details request because contact, rating, hours and photo data are irrelevant to a search origin.
