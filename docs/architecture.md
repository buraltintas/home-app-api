...
| Profiles | one-to-one `user_profiles` (public), `user_private_profiles` (private JSONB/optional demographics) |
| Anonymous | `visitor_sessions(id, linked_user_id, expires_at)`; its ID is analytics correlation, never authentication |
| Stores | `stores` with `geography(Point,4326)` and GiST index; `store_categories`, links; `store_external_sources(provider,external_id unique)`; one-to-one `store_stats` |
| Social | `posts` with rating check 1..5 and derived visit distance/time; `media`, ordered `post_media`; unique favorites/likes/follows; one-level `comments`; self-follow check |
| Search | `searches` with validated intent JSONB and privacy-rounded request coordinates; `search_results(search_id,rank unique)`; idempotent `search_interactions` |
| Delivery | `email_outbox` and `email_deliveries`; `push_devices`, `notification_preferences`, `notification_outbox` |
