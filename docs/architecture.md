...
| Stores | language-independent `stores` with `geography(Point,4326)`, GiST index, and an administrator-owned `is_catalog_store` editorial marker; canonical category keys plus `store_category_translations`; optional descriptive `store_translations`; `store_external_sources(provider,external_id unique)`; one-to-one `store_stats` |
...
The catalogue marker is deliberately absent from this ordering. It lets an administrator
identify a curated catalogue store and lets clients present that fact consistently, but it
is neither paid placement nor a relevance signal. Changing it creates an audit event and
does not change categories, premium status, or result rank.
