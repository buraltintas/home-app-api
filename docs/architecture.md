...
| Social | `posts` and `comments` preserve original UGC plus optional `content_language`; rating and derived visit distance/time; `media`, ordered `post_media`; unique favorites/likes/follows; self-follow check |
...
Important indexes cover provider identity, active sessions, feed `(created_at,id)`, user/store posts, all social uniqueness keys, search owner/history, result rank, outbox availability, and geographic GiST lookup. Exact submitted review coordinates are not retained; only distance and verification time are stored. Search coordinates are rounded according to configuration and can be deleted/anonymized by retention jobs.
...
OpenAI parsers emit the same canonical category/product/style/price identifiers.
...
Only the raw query plus coarse locale/location context is sent. The official OpenAI Go SDK calls the Responses API with a strict JSON schema for categories, products, styles, price intent, attributes, location and sort preference. The response is size-bounded, decoded with unknown-field rejection, enumerations/lengths/radius/coordinates are validated, and it is merged only into fields allowed by the deterministic parser.
